...
the acquisition policy, mechanised. Fetches from the structured source, renders it,
...
def uslm_section(identifier):
    """The section designator out of a USLM identifier: /us/usc/t15/s1681g/a -> 1681g.

    A LEAF MUST KNOW ITS OWN SECTION. This walker used to leave it unset, on the reasoning that a
    single-section segmentation needs no section filter — true only while the comparison treated a
    null section as matching everything. That permissive branch is what let 2,431 leaves ship
    without one, and with it every check that scopes by section either passed silently or found
    nothing silently. engine/requirements.mjs reported zero elements for every U.S. Code provision
    as a result. Gate 48 now refuses a segmentation whose leaves lack it.

    The designator is stored rather than the whole identifier so it matches a plain anchor like
    "227"; consumers normalise the two forms before comparing. USLM writes hyphenated sections
    with an en dash, which is normalised here so 1320d-5 and 1320d–5 are one section.
    """
    m = re.match(r'^/us/usc/t[^/]+/s([^/]+)', str(identifier or ''))
    if not m:
        return None
        ...
        for ch in el:
            ...
            if cont is not None and not kids:
                out.append(dict(path=p2, anchor=ch.get('identifier'),
                                section=uslm_section(ch.get('identifier')),
        ...
        if ps:
            ...
            if os.environ.get('USLM_RECONSTRUCT_FLAT') == '1':
                ...
                for l in lv:
                    ...
                    # The synthetic DIV8 wrapper labels leaves from the identifier, which used
                    # to disagree with the USLM anchor an atom cites — so this dropped the label
                    # rather than invent one. Dropping it was the worse trade: a null section
                    # matched everything, which is how 2,431 leaves shipped without one. The two
                    # anchor forms are now reduced to a section designator before comparison, so
                    # the label can be kept and is the honest value.
                    l['section'] = uslm_section(sec.get('identifier'))
                return lv
            raise SystemExit(
...
if __name__ == '__main__':
    ...
    # LOG TO THE SOURCES LEDGER. DESIGN.md Phase 0 requires every fetch to be recorded in
