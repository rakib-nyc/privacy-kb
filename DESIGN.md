...
The properties below are correctness requirements, not preferences. They are what make the
engine's output checkable, and every one of them is enforced somewhere in `tools/validate.mjs`
or the test suite rather than left to convention.
...
## 3. The analysis order

Steps one to six are deterministic code. A model may narrate the
result; it never computes it.
...
## 4. Failure modes defended against
