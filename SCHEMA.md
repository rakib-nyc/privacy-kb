...
| `privacy_exposure(entity, data, as_of, …)` | what CHANGES if a fact changes: decisions, events, thresholds and multi-fact combinations, each re-computed |
| `privacy_requirements(citations[])` | every element a provision requires, with hashes — plus `not_held[]`, the elements in the source this corpus lacks |
| `privacy_crosswalk(citation?)` | declared correspondences between regimes, each resolved to BOTH provisions' verbatim text and hashes |
...
| `privacy_profile(action, name, …)` | a standing fact set; `check` reports whether the corpus, the facts or the answer moved |
| `privacy_translate(terms[], vocabulary?)` | Fideslang terms into fact keys: assert / conditional / surface / unmapped |
