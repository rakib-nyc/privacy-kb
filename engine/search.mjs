...
export function search(q, opts = {}) {
  // A PARAMETER DEFAULT COVERS `undefined` ONLY. search(q, null) skipped the default and threw on
  // opts.record_type. See tests/test-null-totality.mjs, which now asserts this for every entry
  // point rather than leaving it to be rediscovered one module at a time.
  opts = opts ?? {};
  const corpus = load();
  const query = norm(q);
  ...
  for (const rec of corpus.all) {
    // INVARIANT I1 BEFORE EVERY OTHER FILTER. Search reaches reference text by design, and that
    // made it the easiest place to forget that it must not reach SUPPRESSED text: searching a
    // phrase from an unverified record returned that record, placeholder span and all. The same
    // shape as QA-03 and as the brief defect found alongside this one — a surface that reads
    // corpus.all inherits the exposure unless it filters, every time.
    if (!surfaceable(rec)) continue;
