...
export function backstops(entityFacts, context) {
  // INVARIANT I6: BACKSTOPS NEVER TURN OFF. Throwing on null facts is one of the ways they turn
  // off — the caller gets a stack trace where it should get the FTC Act and the state UDAP layer.
  // A parameter default would not have covered it, because there is none here and null skips one
  // anyway. A caller with no facts still gets the backstops that apply to everyone.
  entityFacts = entityFacts ?? {}; context = context ?? {};
  const corpus = load();
  const out = [];
