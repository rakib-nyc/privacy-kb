...
export function inForceOn(a, asOf) {
  // A null record is not in force. See surfaceable() above: these two predicates are called
  // over every record in the corpus, so neither may assume its argument exists.
  if (!a) return false;
...
// Total on null BY DESIGN, not by accident. This is the invariant I1 guard, and every surface
// that reads corpus.all filters through it; a null record must read as NOT surfaceable rather
// than throw, because a crash here would be a suppressed record escaping as a stack trace.
