#!/usr/bin/env node
// Property tests for the engine. the acquisition policy; the rest guard invariants that
...
// rather than a silent pass.
{
  ...
  // DEBT-025: a category is satisfied by ONE citation matching its prefix, so "present" is a claim
  ...
  ok('DEBT-025: a prefix-present category is measured against its segmentation',
     fcra.element_coverage?.thin?.some(entry => entry.prefix === '15 U.S.C. § 1681g'
       && entry.expected > 100 && entry.held < entry.expected),
     JSON.stringify(fcra.element_coverage?.thin?.find(e => /1681g/.test(e.prefix)) ?? null));
  ok('DEBT-025: and the summary says completeness above is about prefixes, not elements',
     /claim about prefixes, not about elements/.test(fcra.summary));

  // An unmeasurable denominator must READ DIFFERENTLY from a satisfied one. "34 C.F.R. § 99" is a
  // part, not a section, so no single segmentation covers it; reporting a shortfall of zero there
  // would be the absence-against-what-happens-to-be-there error one level further down.
  const ferpa = cov.instrumentCoverage('us.ferpa', corpus);
  ok('DEBT-025: a part-level prefix is declared unassessed, never reported as complete',
     ferpa.element_coverage?.unassessed?.some(entry => entry.prefix === '34 C.F.R. § 99' && entry.reason),
     JSON.stringify(ferpa.element_coverage?.unassessed ?? null));

  // The caveat is worthless if it cannot say WHICH quantity fell short. 42 U.S.C. § 12112(d)(3)(B)
  // is held as an obligation while its three exceptions are not, and "holds 0 of 3" read as
  // "nothing is here" until the sentence named the sub-elements and whether the prefix itself is held.
  const ada = cov.instrumentCoverage('us.ada', corpus);
  const conf = ada.element_coverage?.thin?.find(entry => entry.prefix === '42 U.S.C. § 12112(d)(3)(B)');
  ok('DEBT-025: a thin entry records whether the prefix ITSELF is held',
     conf?.prefix_itself_held === true, JSON.stringify(conf ?? null));
  ok('DEBT-025: and the prose counts sub-elements explicitly',
     /beneath it/.test(ada.summary), ada.summary.slice(-160));

  // CORPUS COVERAGE IS NOT ANSWER COVERAGE. Both sites in applicability.mjs gated this caveat on
  // `complete`, which cannot see a thin category by construction — so the shortfall was computed
  // here and dropped on the way out. Asserting it on the module would have passed while every
  // answer stayed silent, which is why this asserts on analyze() instead.
  const answer = analyze({ nexus: 'US', is_consumer_reporting_agency: true }, {},
    { as_of: '2026-09-16', law: { federal_instrument: 'us.fcra', state_instrument: 'ny.gbl.349' } });
  ok('DEBT-025: an ANSWER over a thin instrument carries the shortfall in coverage_gaps',
     (answer.coverage_gaps ?? []).some(line => /prefixes, not about elements/.test(line)),
     JSON.stringify((answer.coverage_gaps ?? []).slice(0, 2)));
  const completeness = (answer.applicable ?? [])
    .map(entry => entry.instrument_completeness).filter(Boolean);
  ok('DEBT-025: ...and the applicable entry names the thin prefixes',
     completeness.some(entry => (entry.element_coverage?.thin ?? [])
       .some(t => t.prefix === '15 U.S.C. § 1681g')),
     `${completeness.length} entr(ies) carried completeness`);
}
