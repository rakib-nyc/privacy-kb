...
// ---------------------------------------------------------------- the memo is REPRODUCIBLE
// The memo is the defensibility record, and its value rests on being able to regenerate it and
// compare. Nothing asserted that. engine/requirements.mjs has a byte-identical check; the
// flagship deliverable did not, so the one claim a reader would actually test was untested.
//
// buildMemo takes generated_at as its FOURTH argument, not in the context. Passing it in the
// context leaves the wall clock in the document and two runs milliseconds apart differ — which is
// how a reproducibility check comes to report a defect that is the caller's.
{
  const facts = { nexus: 'US', is_hipaa_covered_entity: true, is_consumer_reporting_agency: true };
  const context = { as_of: '2026-09-16' };
  const opts = { generated_at: '2026-09-16T00:00:00.000Z' };
  const digest = value => createHash('sha256').update(JSON.stringify(value)).digest('hex');

  const runs = [buildMemo(facts, {}, context, opts), buildMemo(facts, {}, context, opts),
                buildMemo(facts, {}, context, opts)].map(digest);
  ok('three memos over the same inputs are byte-identical', new Set(runs).size === 1,
     runs.map(x => x.slice(0, 12)).join(' '));

  // A FRESH PROCESS IS THE HARDER TEST. Module-level caches make one process agree with itself,
  // and every read here — the corpus, the segmentations, the declarations — is memoised.
  const script =
    "import{buildMemo}from'./engine/memo.mjs';import{createHash}from'node:crypto';" +
    "const m=buildMemo(" + JSON.stringify(facts) + ",{}," + JSON.stringify(context) + "," +
    JSON.stringify(opts) + ");" +
    "process.stdout.write(createHash('sha256').update(JSON.stringify(m)).digest('hex'));";
  const out = [0, 1].map(() => execFileSync('node', ['--input-type=module', '-e', script],
    { cwd: resolve(import.meta.dirname, '..'), encoding: 'utf8' }));
  ok('...and a memo built in a FRESH PROCESS has the same digest',
     out[0] === out[1] && out[0] === runs[0],
     `${out[0].slice(0, 12)} ${out[1].slice(0, 12)} vs in-process ${runs[0].slice(0, 12)}`);

  // Unpinned, the timestamp is the ONLY thing that may move. If anything else does, the memo is
  // carrying state it has not declared.
  const a = buildMemo(facts, {}, context), b = buildMemo(facts, {}, context);
  const strip = memo => JSON.stringify({ ...memo,
    record: { ...(memo.record ?? {}), generated_at: null },
    markdown: String(memo.markdown ?? '').replace(/Generated \S+/g, 'Generated <pinned>') });
  ok('an unpinned memo differs ONLY in its timestamp', strip(a) === strip(b));
}
